...
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Zeus
...
download the file. Save it somewhere only Zeus can read (e.g.,
...
Also grant `Pub/Sub Viewer` on the same subscription — Zeus calls
...
- **App name**: whatever you want users to see ("Zeus" is reasonable).
...
`ADDED_TO_SPACE` event that Zeus uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Zeus
...
Install the dependencies the Google Chat adapter needs (no Zeus extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Zeus is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Zeus detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Zeus session.
...
3. Download the JSON. Move it onto the host that runs Zeus.
4. Register the client with Zeus (run under the profile you want it scoped to):
...
That writes the client secret into the active profile's Zeus home (e.g.
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Zeus isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
- **Attachment download protection**: Zeus will only attach the SA bearer
