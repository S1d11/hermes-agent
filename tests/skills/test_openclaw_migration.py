...
def test_rebrand_text_replaces_openclaw_variants():
    ...
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Zeus prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Zeus to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Zeus config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Zeus uses tools well"
    # All-lowercase matches → lowercase ``zeus``; this preserves the
    # real filesystem path ``~/.hermes`` (Zeus home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "zeus should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Zeus remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "zeus prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Zeus was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "zeus uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Zeus said to ask Zeus about Zeus settings"
