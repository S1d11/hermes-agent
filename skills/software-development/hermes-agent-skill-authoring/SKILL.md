...
# Authoring Zeus-Agent Skills (in-repo)
...
- You're committing a reusable workflow that should ship with Zeus
...
2. **Choose the right context load.** A model-invoked Zeus skill pays for its description every turn. Keep descriptions focused on trigger classes and the skill's distinctive behavior. Put details in the body or linked references.
...
- Zeus-specific recipes (tests via scripts/run_tests.sh, ui-tui paths, etc.)
