...
Connect Zeus to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Zeus — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Zeus runs it in a
...
Photon uses the same authorization model as every other Zeus
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Zeus replies with a pairing code. Approve it with:
...
By default Zeus responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Zeus` and
...
Send an iMessage to your assigned number and Zeus will reply.
...
assigned iMessage line Zeus uses. When a Photon token and dashboard project
...
- **Outbound attachments are supported.** Zeus sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Zeus wake words  | JSON list / comma / newline regex patterns for group mentions |
